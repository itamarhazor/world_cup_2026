...
Using web search, collect current data for the following 16 UEFA teams:
...
For each team output EXACTLY this JSON object (format example only — do not output Spain in the output array, it must appear as a real data object):
...
- last_tournament_stage: most recent major tournament result, e.g. "Winner Euro 2024", "SF Euro 2024", "Group Stage WC 2022". Use "Did not qualify" if they missed the last tournament
...
- confederation_form: MUST be one of → "dominant" | "strong" | "average" | "weak" — based on how UEFA teams performed in the last 18 months of international football
- host_boost: false for all UEFA teams (none are co-hosts)
...
Output ONLY a valid JSON array of all 16 objects.
